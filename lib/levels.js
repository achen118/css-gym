...
class Level {
  ...
  goToLevel(levelNo) {
    ...
    if (levelNo === 5) {
      ...
      levelDescription.innerHTML = "<h4>COMBINATORS:<br />Child Selectors</h4><br />The child combinator (>) separates two selectors and matches only those elements matched by the second selector that are children of elements matched by the first. In contrast, when two selectors are combined with the descendant selector, the combined selector expression matches those elements matched by the second selector for which there exists an ancestor element matched by the first selector, regardless of the number of 'hops' up the DOM.<br /><br /><h5>Syntax</h5><br /><code>selector1 > selector2 { style properties }</code><br /><h5>CSS</h5><br /><code>span { background-color: white; }<br />div > span {<br />  background-color: DodgerBlue;<br />}</code><br /><h5>HTML</h5><br /><code>&#60;div><br />  &#60;span>Span #1, in the div.<br />    &#60;span>Span #2, in the span that's in the div.&#60;/span><br />  &#60;/span><br />&#60;/div><br />&#60;span>Span #3, not in the div at all.&#60;/span></code><br /><h5>Result</h5><br /><code><div><span5>Span #1, in the div.<span5>Span #2, in the span5 that's in the div.</span5></span5></div><span5>Span #3, not in the div at all.</span5></code>";
      gymBench.innerHTML = '\n  <dumbbell class="small"></dumbbell>\n  <kettlebell></kettlebell>\n  <dumbbell class="small"></dumbbell>\n  <step>\n    <kettlebell></kettlebell>\n  </step>\n';
      levelHTML.innerHTML = document.querySelector('.gym-bench').outerHTML;
      document.querySelectorAll('dumbbell').forEach(el => el.classList.add('move-left'));
    }
  ...
  hoverSelector() {
      document.addEventListener("mouseover", event => {
        const visibleMark = document.querySelector('.visible');
        if (visibleMark) {
