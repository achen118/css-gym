...
class Level {
  ...
  hoverSelector() {
    document.querySelectorAll(this.currentSelector).forEach(el => {
      el.addEventListener("mouseenter", event => {
        this.mark.classList.add('visible');
        if (event.target === document.querySelector('dumbbell')) {
          this.mark.classList.remove('right');
          this.mark.classList.add('left');
        } else {
          this.mark.classList.remove('left');
          this.mark.classList.add('right');
        }
      });
